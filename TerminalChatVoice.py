...
def query_ollama(input_text):
    ...
    ollama_api_key = Path('ollama_api_key_file.txt').read_text()
    headers = {'Content-Type': 'application/json', 'Authorization': f'Bearer {ollama_api_key}'}
    # payload = {"model": "llama3.1:8b", "prompt": input_text}
    payload = {"model": "gpt-oss:120b-cloud", "prompt": input_text}
    try:
        # Stream the response, Enables streaming the response incrementally instead of loading it all at once.
        ...
        for line in response.iter_lines(decode_unicode=True):            #It reads the response line by line
            if line:
                # function takes a JSON-formatted string (the line) and converts it into a Python dictionary
                data = json.loads(line)
                full_response += data.get("response", "")
                # Checks if the "done" field in the JSON is True, signaling the end of the response.
